...
export default function ClozePage() {
  ...
  async function buildGame() {
    ...
    const qs: ClozeQuestion[] = selected.map((word) => {
      ...
      return { word, blankedSentence, options, correctIndex };
    });
    ...
    return (
      <div className="min-h-dvh flex flex-col items-center justify-center px-6 pb-28" style={{ background: "var(--bg)" }}>
        <div className="text-center bounce-in w-full max-w-sm">
          <div className="text-7xl mb-4">{notEnough ? "📖" : acc >= 80 ? "🏆" : acc >= 60 ? "⚡" : "💪"}</div>
          <h2 className="text-3xl font-black uppercase tracking-tight mb-1" style={{ color: "var(--text)" }}>
            {notEnough ? "Cần câu ví dụ!" : "Cloze hoàn thành!"}
          </h2>
          <p className="mb-6 font-medium" style={{ color: "var(--muted)" }}>
            {notEnough ? "Thêm từ có câu ví dụ để chơi Cloze." : `${sessionCorrect}/${total} đúng`}
          </p>
    ...
    <div className="min-h-dvh flex flex-col pb-28" style={{ background: "var(--bg)" }}>
      <div className="px-4 pt-safe pt-4 pb-2">
        ...
        <div className="h-3 overflow-hidden" style={{ background: "var(--surface2)", border: "1.5px solid var(--border)", borderRadius: "2px" }}>
          <div className="h-full transition-all duration-500" style={{ width: `${progress}%`, background: "var(--green)" }} />
        </div>
      </div>
      ...
      {current && (
        <div className="flex-1 flex flex-col justify-between px-4 py-4">
          ...
          <div
            ...
            style={{
              background: answerState === "correct" ? "#2a6040" : answerState === "wrong" ? "var(--primary)" : "var(--surface)",
              border: "2px solid var(--border)", boxShadow: "5px 5px 0 var(--border)", borderRadius: "4px",
            }}
          >
            <p className="text-xs font-black mb-4 uppercase tracking-widest" style={{ color: answerState !== "idle" ? "rgba(248,243,234,0.6)" : "var(--muted)" }}>
              📝 Điền từ còn thiếu vào chỗ trống
            </p>
            <p className="text-lg font-semibold leading-relaxed" style={{ color: answerState !== "idle" ? "#f8f3ea" : "var(--text)" }}>
              {answerState !== "idle"
                ? current.word.example
                : current.blankedSentence.split("______").map((part, i, arr) => (
                    <span key={i}>
                      {part}
                      {i < arr.length - 1 && (
                        <span className="font-black px-1 mx-0.5" style={{
            ...
            {answerState !== "idle" && current.word.translation && (
              <p className="mt-3 text-sm font-medium" style={{ color: "rgba(248,243,234,0.7)" }}>
